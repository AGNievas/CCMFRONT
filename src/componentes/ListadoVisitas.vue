<template>
    <v-dialog v-model="isDialogVisible" persistent max-width="1200px">
        <v-card>
            <v-card-title>
                <span class="headline">Historial Visitas</span>
            ...
            <Tabla :data="valoresTabla" :headers="visitasHeader" :isListadoVisitas="true" @darAlta="openAltaDialog"
                @edit="openEditarVisitaDialog" @delete="confirmDeleteVisita"  @ver-historial="openListadoApliques" @crear-aplique="openApliqueDialog"/>

         <v-dialog persistent v-model="apliqueDialogVisible" max-width="600px">
        <ApliqueDialog v-model="apliqueDialogVisible" :paciente="paciente" :visita="selectedVisita" :areas="globalStore.getAreas"
       ...
       <ListadoApliques v-if="listadoApliquesVisible" v-model="listadoApliquesVisible" :visita="selectedVisita"
        :areas="globalStore.getAreas" :usuarios="globalStore.getUsuarios" :paciente="paciente" :medicamentos="medicamentos" /> 

    

            <VisitasDialog v-if="visitaDialogVisible" v-model="visitaDialogVisible" :isEditing="isEditing"
                :visita="selectedVisita" :paciente="paciente" :usuarios="usuarios" :darAlta="darAlta"
                :stockAreas="stockAreas" :areas="areas" @save="saveVisita" :errorMensaje="errorMensaje"
