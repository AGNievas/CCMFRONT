<template>
    ...
    <v-card class="custom-container">
        <v-card-title class="d-flex align-center pe-2"> {{ darAlta ? 'Ingrese Fecha de Alta' :isEditing ? 'Editar Visita' : 'Agregar Visita' }}

        </v-card-title>
            <v-card-text>
                <v-select v-if="!darAlta" v-model="visitaLocal.StockArea" :items="stockAreasPorArea" item-title="nombre" item-value="id"
                    label="Sub Area" required />

                <v-select v-if="!darAlta" v-model="visitaLocal.User" :items="usuariosPorAreas" item-title="fullName" item-value="id"
                    label="Medico" required />
                ...
                <v-select v-if="!darAlta" v-model="visitaLocal.seInterno" :items="opciones" item-title="text" item-value="valor"
                    label="¿Requiere Internación?" outlined />

                <v-text-field v-if="darAlta || isEditing" v-model="visitaLocal.fechaAlta" label="Fecha Alta" type="datetime-local" />
            </v-card-text>
         <v-card-actions>
        <v-spacer></v-spacer>
