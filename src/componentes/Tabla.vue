<template>
  <v-data-table :headers="headers" :items="sortedData" hide-default-header v-if="!isMobile">
    <template v-slot:thead>
      <thead>
        <tr>
          <th :class="{ sorted: sortKey === header.value }" class="table-header celdas"
            v-for="(header, index) in headers" :key="index" @click="sortTable(header.value)">
            {{ header.text }}
            <v-icon small v-if="index !== headers.length - 1">
              {{ sortKey === header.value ? (sortOrder === 'asc' ? 'mdi-sort-ascending' : 'mdi-sort-descending') :
              'mdi-sort' }}
            </v-icon>
          </th>
        </tr>
    ...
    <template v-slot:item="{ item }">
      <tr>
        ...
        <td class="text-start acciones-cell">
          <AccionButtons :item="item" :editable="editable" :eliminable="eliminable" :canVerEdit="canVerEdit"
            :canVerDelete="canVerDelete" :canRestorePassword="canRestorePassword" :canCreateAplique="canCreateAplique"
            :canVerHistorial="canVerHistorial" :canVerDetail="canVerDetail" :canVerVisitas="canVerVisitas"
            :puedeDarAlta="puedeDarAlta" @edit="$emit('edit', item)"
            @delete="$emit('delete', isListadoMedicamentos ? item : item.id ? item.id : item.sku ? item.sku : index)"
            @restorePassword="$emit('restorePassword', item)" @ver-historial="$emit('ver-historial', item)"
            @crear-aplique="$emit('crear-aplique', item)" @ver-items="$emit('ver-items', item.id)"
            @ver-visitas="$emit('ver-visitas', item)" @dar-alta="$emit('dar-alta', item)" />
        </td>
      </tr>
    </template>
  </v-data-table>
  ...
  <div style="max-height: 600px; overflow-y: auto;" v-if="isMobile">
    <v-card class="mx-auto" max-width="400" v-for="(item, index) in sortedData" :key="index"
  ...
  computed: {
    ...
    canVerEdit() {
      return ((this.isListadoUsuarios && this.globalStore.getRolId == this.globalStore.getRolSuperAdmin) ||
        ...
        this.isListadoPacientes || this.isListadoApliques ||
        ...
        this.isListadoVisitas) || this.isEditable;
    },
